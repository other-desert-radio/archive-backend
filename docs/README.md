...
- `bun run db:export:archive` creates missing cached DJ WebP derivatives, then
  writes DJ detail/index JSON, the top-level show index, tags, and DJ WebP
  images to the configured local Astro archive directories. It logs each build
  stage and generated file.
