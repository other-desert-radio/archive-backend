...
- Broadcast date has day-level granularity only. Accept YYYY-MM-DD, store
...
- [ ] Save this plan as docs/SHOWS_IMPLEMENTATION_PLAN.md when implementation
      begins.
- [ ] Maintain its checkboxes throughout implementation. Distinguish
      “implemented,” “verified,” and “reviewed”; never mark user review complete
      automatically.
- [ ] Read current repository guidance before each chunk, including API
      conventions before route changes.
- [ ] Preserve unrelated work. Initial inspection found an untracked src/res/
      directory; do not modify or delete it.
- [ ] Update relevant documentation in the same chunk as behavior changes.
- [ ] Complete each chunk’s tests and browser verification, report results and
...
- [ ] Extract a resource-neutral toolbar accepting search value/change callback,
      accessible search label, create-button label/callback, and optional
      disabled create state.
- [ ] Preserve the current search placeholder, disabled grid button, selected
      table button, spacing, and square create button.
- [ ] Extract a typed generic table using column definitions rather than
      hardcoded DJ markup.
- [ ] Each column definition supplies a stable key, label, cell renderer, and
      comparator. The table receives rows, row-key accessor, caption, current
      sort, and sort callback.
- [ ] Share filtering and sorting mechanics: trimmed case-insensitive substring
      matching, sorting a copied array, and active-column toggling. Resource
      adapters provide search text and comparators.

- [ ] Preserve DJ sorting exactly: ID descending initially; new column
      ascending; active column toggles; relationship columns compare their first
      ID with the existing empty-value behavior.

- [ ] Keep existing DJ search coverage and sanitized HTML rendering intact.
      Extract shared date, missing-value, and relationship-ID presentation where
      useful.
- [ ] Share loading, retry, empty-dataset, and no-search-results presentation.
      Keep the toolbar available for an empty successfully loaded dataset.
- [ ] Add an explicit “No DJs match your search” state rather than rendering a
      blank body.
- [ ] Use resource-neutral CSS classes for shared elements. Preserve borders,
...
- [ ] Extract a generic onboarding modal responsible for overlay, dialog
      heading, close control, form layout, error area, Submit button, and
      pending state.
- [ ] Extract resource-neutral labeled input/textarea controls supporting IDs,
      input types, required state, helper text, errors, and disabled state.
- [ ] Keep resource form values, normalization, and validation in their own form
      components. Do not build a schema-driven form engine.
- [ ] Extract the existing comma-separated tags field and missing-tag feedback
      into a shared component. Reuse validate-tags; never create tags on blur.
- [ ] Ignore stale tag-validation responses after edits, closure, or reopening.
      A validation-service failure must not prevent final server-side
      submission.
- [ ] Extract shared mutation-error formatting so both loaders show
      resource-specific copy, HTTP status, and readable API errors consistently.
      Keep DJ multipart serialization unchanged.
- [ ] Migrate OnboardDJModal to these primitives. Preserve all current DJ
      fields, image-upload validation, payload construction, and success refresh
      behavior.
- [ ] Keep the DJ image picker resource-specific for now; Shows will use a URL
      input.

Define common modal behavior once:

- Opening starts a fresh form session with cleared errors.
- Close button and Escape close an idle modal; backdrop clicks do not dismiss
  it.
- Focus enters the dialog, stays within it, and returns to the opener on
  closure.
- While submitting, disable fields, Submit, and dismissal to prevent duplicate
  submissions and abandoned in-flight sessions.
- Failure preserves values and shows the error inside the modal.
- Success closes the modal and triggers the resource refresh without resetting
  table search or sort.
- A refresh failure after successful creation is a page-loading error, not a
  creation failure.
- Constrain the panel to available viewport height and make its content
  scrollable. Keep heading, close control, and Submit reachable; preserve the
  undimmed top bar.

### Verification and review gate

- [ ] Run focused shared-helper and existing DJ tests, including unchanged
      multipart requests and image behavior.
- [ ] Verify with agent-browser: DJ sorting, search, no matches, modal
      opening/closing, validation, pending/failure states, and fresh state on
      reopening.
- [ ] Compare before/after table and modal screenshots at desktop, short
      desktop, and narrow widths.
- [ ] Verify Tags navigation and layout remain unchanged.
- [ ] Update documentation describing shared component responsibilities and
...
- [ ] Add an admin-specific Show response type extending the existing Show shape
      with createdAt.
- [ ] Return createdAt from GET /api/admin/shows; it is already selected from
      the database.
- [ ] Keep public archive transformer/export contracts unchanged. Compose the
      admin response around the existing transformer.
- [ ] Update the browser row type so serialized timestamps are strings.
- [ ] Replace the old Shows heading/layout with the shared resource view,
      toolbar, and table.
- [ ] Configure columns in this exact order: id, created at, title, date,
      duration, image, DJs, tags, URL.
- [ ] Render creation timestamps in the same UTC format as DJs.
- [ ] Render broadcast dates as the UTC calendar date only, including existing
      records with non-midnight timestamps. Do not rewrite existing stored
      dates.
- [ ] Render duration as HH:MM:SS, allowing hours above 23.
- [ ] Render image URLs as text, matching DJ image-path presentation; show None
      when absent.
- [ ] Render DJ/tag IDs as comma-separated values with None for empty arrays.
- [ ] Render the show URL as readable linked text for HTTP(S) URLs. Render
      unsupported legacy values as plain text.
- [ ] Show the shared + Show button disabled until onboarding is implemented in
      chunk 3.

### Search and sorting

- [ ] Load Shows, DJs, and Tags to build search text using relationship names as
      well as IDs. Use ID-indexed lookup maps.
- [ ] Search all displayed Show values, including formatted dates/duration,
      URLs, and resolved DJ/tag names.
- [ ] Default to ID descending and use the shared header-toggle behavior.
- [ ] Sort ID and duration numerically, dates chronologically, text
...
- [ ] Treat failure of the required resource/lookups load as a retryable page
      error. Ignore obsolete load results after unmount or a newer refresh.

### Verification and review gate

- [ ] Add focused API-response and frontend tests for createdAt, column
      rendering, search, sort, and empty values.
- [ ] Test numeric sorting, relationship-name searches, case-insensitive
      searches, optional image absence, and no results.
- [ ] Test day rendering across positive and negative browser timezone offsets
      to ensure no calendar-day shift.
...
- [ ] Convert the accepted date to midnight UTC explicitly, without
      browser/server local-time interpretation.
...
- [ ] Run the repository formatter, inspect its diff, then run bun run lint and
      git diff --check.
- [ ] Run the full test suite before the final onboarding handoff.
- [ ] Record agent-browser checks and screenshot locations, identifying
